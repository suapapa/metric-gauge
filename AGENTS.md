...
8. **Env typo**: Primary names are `GUAGE1_PROM_METRIC` / `GUAGE2_PROM_METRIC` and `GUAGE1_ROTATION` / `GUAGE2_ROTATION` (as specified by the project owner). `GAUGE*` and `PASSWORD` are accepted aliases in `build.rs`.
